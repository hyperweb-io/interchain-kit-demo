...
export interface UseRpcClientQuery<TData>
  extends ReactQueryParams<ProtobufRpcClient, TData> {}

export interface UseRpcEndpointQuery<TData>
  extends ReactQueryParams<string | HttpEndpoint, TData> {
...
export function buildUseQuery<TReq, TRes>(
  ...
  return <TData = TRes>({
    ...
    return useQuery<TRes, Error, TData>(
      customizedQueryKey || [opts.queryKeyPrefix, request],
      () => queryFn(request),
      options
    );
  };
...
export interface UseQueryParams<TReq, TRes, TData = TRes>
  extends ReactQueryParams<TRes, TData> {
  request: TReq;
  customizedQueryKey?: QueryKey;
}

export interface ReactMutationParams<
  TData,
  TError,
  TVariables,
  TContext = unknown
...
export interface UseMutationBuilderOptions<TMsg> {
  builderMutationFn: (
    getSigningClientInstance: () => ISigningClient | undefined
  ) => (
...
export function buildUseMutation<TMsg, TError>(
  ...
  return ({
    ...
    return useMutation<DeliverTxResponse, Error, ITxArgs<TMsg>>(
      ...
      options as Omit<
        UseMutationOptions<DeliverTxResponse, Error, ITxArgs<TMsg>, unknown>,
        "mutationFn"
      >
    );
  };
